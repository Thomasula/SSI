...
def f_ped_rep(ped_data, ped_idx, other_idx, const):
   if ped_idx == other_idx:
       ...
       sx = (last_x_other - last_x_ped) / d if d != 0 else 0
       sy = (last_y_other - last_y_ped) / d if d != 0 else 0
...
const = {'N_ped': 10,           # numer of peds in the system - to be set in init function
        'N_step': 100,             # maximum number of model steps
        ...
        'attractor_y': 9,          # y position of attractor [m]
...
# PEDESTRIANS ALWAYS GET STUCK AT (10, 0) or in the near proximity of that point

max_occupancy = 12
occupancy_range = range(max_occupancy - 7, max_occupancy)
iteration_count = 5
N_ped = const['N_ped']
